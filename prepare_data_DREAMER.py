...
class PrepareData:
    def __init__(self, args):
        ...
        self.dreamer = [['AF3'], ['F7', 'F3', 'FC5'], ['T7'], ['P7'], ['O1', 'O2'], ['P8'], ['T8'], ['FC6', 'F4', 'F8', 'AF4']]
    ...
    def run(self, subject_list, split=False, expand=True):
        ...
        for sub in subject_list:
            data_, label_ = self.load_data_per_subject(sub, raw)
    ...
    def load_data_per_subject(self, sub, raw):
        """
        ...
        data: (18, 14, ?) label: (18, 3)
        """
        # data = (raw["DREAMER"][0, 0]["Data"][0, sub]["EEG"][0, 0]["stimuli"][0, 0][0, 0][:, ])
        # subject = np.zeros((18, 107520))
        data = []
        # video_len = []
        label = []
        for video in range(0, 18):
            # for electrode in range(0, 14):
            stim = raw["DREAMER"][0, 0]["Data"][0, sub]["EEG"][0, 0]["stimuli"][0, 0][video, 0][-7680:, :]
            label_V = int(raw["DREAMER"][0, 0]["Data"][0, sub]["ScoreValence"][0, 0][video, 0])
            label_A = int(raw["DREAMER"][0, 0]["Data"][0, sub]["ScoreArousal"][0, 0][video, 0])
            label_D = int(raw["DREAMER"][0, 0]["Data"][0, sub]["ScoreDominance"][0, 0][video, 0])
            label.append([label_V, label_A, label_D])
            data.append(stim.T)
            # video_len.append(stim.shape[0])
...
def feat_extract_EEG(raw):
    ...
    for participant in range(0, 23):
        for video in range(0, 18):
            ...
            for electrode in range(0, 14):
                stim = (
                raw["DREAMER"][0, 0]["Data"][0, participant]["EEG"][0, 0]["stimuli"][0, 0][video, 0][:, electrode])
